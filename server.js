...
function orderReceivedEmailHtml(order) {
  return `
    ...
    <p>You can track your order on the website using this order ID.</p>
  `;
}
...
function paymentSuccessEmailHtml(order) {
  return `
    ...
    <p>Your VPS details will be sent within 24 hours.</p>
  `;
}
...
function deliveryEmailHtml(order) {
  ...
  return `
    <h2>Your VPS Details</h2>
    <p>Your service is ready.</p>
    <p><strong>Order ID:</strong> ${order.id}</p>
    <p><strong>Product:</strong> ${order.productName}</p>
    <p><strong>Username:</strong> ${credentials.username || ""}</p>
    <p><strong>Password:</strong> ${credentials.password || ""}</p>
    <p><strong>IP Address:</strong> ${credentials.ipAddress || ""}</p>
    <p><strong>Delivery Method:</strong> ${order.deliveryMethod || "Email"}</p>
  `;
}
