...
function ensureDataFiles() {
  try {
    ...
    if (error && (error.code === "EROFS" || error.code === "EPERM")) {
      if (!memoryProducts) {
        memoryProducts = DEFAULT_PRODUCTS.map((product) => ({ ...product }));
      }
      if (!memoryOrders) {
        memoryOrders = [];
      }
...
function readJson(filePath) {
  if (memoryProducts && filePath === PRODUCTS_FILE) {
    return memoryProducts;
  }
  if (memoryOrders && filePath === ORDERS_FILE) {
    return memoryOrders;
  }
  ...
  try {
    ...
    if (error && (error.code === "EROFS" || error.code === "EPERM")) {
      if (filePath === PRODUCTS_FILE) {
        memoryProducts = Array.isArray(data) ? data.map((item) => ({ ...item })) : [];
        return;
      }
      if (filePath === ORDERS_FILE) {
        memoryOrders = Array.isArray(data) ? data.map((item) => ({ ...item })) : [];
        return;
      }
    }
    throw error;
